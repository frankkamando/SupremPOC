...
namespace Suprema
{
    public class LogControl : FunctionModule
    {
        ...
        public void AddEventLogs(BS2Event eventLog)
        {
            ...
            {
                string eventName = Enum.GetName(typeof(BS2EventCodeEnum), (BS2EventCodeEnum)eventLog.code)??"";
                DateTime eventTime = Util.ConvertFromUnixTimestamp(eventLog.dateTime);
                string user = Encoding.UTF8.GetString(eventLog.userID).TrimEnd('\0');
                sqlCon1.Open();

                SqlCommand sql_cmnd = new SqlCommand("AddEventLogs", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;

                sql_cmnd.Parameters.AddWithValue("@id", (int)eventLog.id);
                sql_cmnd.Parameters.AddWithValue("@dateTime", eventTime);
                sql_cmnd.Parameters.AddWithValue("@deviceID", (int)eventLog.deviceID);
                sql_cmnd.Parameters.AddWithValue("@code", (int)eventLog.code);
                sql_cmnd.Parameters.AddWithValue("@msg", Util.GetLogMsg(eventLog)); 
                sql_cmnd.Parameters.AddWithValue("@msgCode", eventName);
                sql_cmnd.Parameters.AddWithValue("@UserID", user);
        ...
        void syncLocalAndDeviceLog(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            {
                string oString = "SELECT top 1 * FROM log ORDER BY id DESC";
                using (SqlCommand cmde = new SqlCommand(oString, con))
                {
                    //cmd.CommandType = CommandType.StoredProcedure;
                    //cmd.Parameters.AddWithValue("@userIDParam", usertarget);
                    con.Open();
                    using (SqlDataReader oReader = cmde.ExecuteReader())
                    {
                        while (oReader.Read())
                        {
                            lastEventId = Convert.ToUInt32(oReader["id"]);
                        }
                        con.Close();
                    }
                }
