...
namespace Suprema
{
    ...
    class DataBaseHandler : IDisposable
    {
        ...
        //{
        //    userList.Clear();
        //    using (SqlConnection con = new SqlConnection(SqlconString1))
        //    {
        //        string oString = "SELECT userID, formatVersion, flag, version, authGroupID, faceChecksum, numCards, numFingers, numFaces FROM BS2User";
        ...
        //                        BS2User user = Util.AllocateStructure<BS2User>();
        //                        Array.Clear(user.userID, 0, BS2Environment.BS2_USER_ID_SIZE);
        //                        string userID = (string)oReader[0];
        //                        byte[] uid = Encoding.UTF8.GetBytes(userID);
        //                        Array.Copy(uid, 0, user.userID, 0, uid.Length);
        //                        user.formatVersion = Convert.ToByte(oReader[1]);
        //                        user.flag = Convert.ToByte(oReader[2]);
        //                        user.version = Convert.ToUInt16(oReader[3]);
        //                        user.authGroupID = Convert.ToUInt32(oReader[4]);
        //                        user.faceChecksum = Convert.ToUInt32(oReader[5]);

        //                        if (Convert.ToBoolean(deviceInfo.cardSupported))
        //                        {
        //                            user.numCards = Convert.ToByte(oReader[6]);
        //                        }
        //                        else
        //                        {
        //                            user.numCards = 0;
        ...
        //                        if (Convert.ToBoolean(deviceInfo.fingerSupported))
        //                        {
        //                            user.numFingers = Convert.ToByte(oReader[7]);
        //                        }
        //                        else
        //                        {
        //                            user.numFingers = 0;
        ...
        //                        if (Convert.ToBoolean(deviceInfo.faceSupported))
        //                        {
        //                            user.numFaces = Convert.ToByte(oReader[8]);
        //                        }
        //                        else
        //                        {
        //                            user.numFaces = 0;
        ...
        //        var startTime = rdr[0];
        //        var endTime = rdr[1];
        //        var fingerAuthMode = rdr[2];
        //        var cardAuthMode = rdr[3];
        //        var idAuthMode = rdr[4];
        //        var securityLevel = rdr[5];
        ...
        //        while (rdr.Read())
        //        {
        //            var fingerIndex1 = rdr[0];
        //            var fingerFlag1 = rdr[1];
        //            var templateData1 = Util.GetBase64frombyte((byte[])rdr[2]);
        //            var templateData2 = rdr[2];
        ...
        public bool GetUserBlob(ref BS2SimpleDeviceInfo deviceInfo, ref BS2User targetUser, ref BS2UserBlob userBlob)
        {
            ...
            string usertarget = Encoding.UTF8.GetString(targetUser.userID);

            userBlob.setting.startTime = Convert.ToUInt32(1652544857);
            userBlob.setting.endTime = Convert.ToUInt32(1652544858);
            userBlob.setting.fingerAuthMode = Convert.ToByte(255);
            userBlob.setting.cardAuthMode = Convert.ToByte(255);
            userBlob.setting.idAuthMode = Convert.ToByte(255);
            userBlob.setting.securityLevel = Convert.ToByte(5);            
            ...
            if (Convert.ToBoolean(deviceInfo.userNameSupported))
            {
                using (SqlConnection con = new SqlConnection(SqlconString1))
                {
                    string oString = "SELECT userName from BS2UserName WHERE userID = @userIDParam";
                    using (SqlCommand cmd = new SqlCommand(oString, con))
                    {
                        //cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@userIDParam", usertarget);
                        con.Open();
                        using (SqlDataReader oReader = cmd.ExecuteReader())
                        {
                            while (oReader.Read())
                            {
                                byte[] userName = Encoding.UTF8.GetBytes((string)oReader[0]);
                                Array.Copy(userName, 0, userBlob.name, 0, userName.Length);
                            }
        ...
        String SqlconString1 = "Data Source=192.168.2.245;Initial Catalog=DemoPoc;Integrated Security=false;Uid=frank;Pwd=12345";
        public void AddUser1(int ID, string NAME,string start,string end)
        {
            using (sqlCon1 = new SqlConnection(SqlconString1))
            {
                sqlCon1.Open();
                SqlCommand sql_cmnd = new SqlCommand("AddUser", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;
                sql_cmnd.Parameters.AddWithValue("@UserID", SqlDbType.Int).Value = ID;
                sql_cmnd.Parameters.AddWithValue("@Username", SqlDbType.NVarChar).Value = NAME;
                sql_cmnd.Parameters.AddWithValue("@startTime", SqlDbType.NVarChar).Value = start;
                sql_cmnd.Parameters.AddWithValue("@endTime", SqlDbType.NVarChar).Value = end;
                sql_cmnd.ExecuteNonQuery();
                sqlCon1.Close();
            }
        }

        public void AddPin(int ID, byte[] pin)
        {
            using (sqlCon1 = new SqlConnection(SqlconString1))
            {
                sqlCon1.Open();
                SqlCommand sql_cmnd = new SqlCommand("AddPin", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;
                sql_cmnd.Parameters.AddWithValue("@UserID", SqlDbType.Int).Value = ID;
                sql_cmnd.Parameters.AddWithValue("@Pin", SqlDbType.Binary).Value = pin;
                sql_cmnd.ExecuteNonQuery();
                sqlCon1.Close();
            }
        }

        public void AddPhotoImage(int ID, int PhotoSize,byte[] PhotoData)
        {
            using (sqlCon1 = new SqlConnection(SqlconString1))
            {
                sqlCon1.Open();
                SqlCommand sql_cmnd = new SqlCommand("AddPhotoImage", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;
                sql_cmnd.Parameters.AddWithValue("@UserID", SqlDbType.Int).Value = ID;
                sql_cmnd.Parameters.AddWithValue("@PhotoSize", SqlDbType.Int).Value = PhotoSize;
                sql_cmnd.Parameters.AddWithValue("@PhotoData", SqlDbType.Binary).Value = PhotoData;
                sql_cmnd.ExecuteNonQuery();
                sqlCon1.Close();
            }
        }

        public void AddFingerPrint(int ID, byte fingerIndex, byte flag, BS2FingerprintTemplateFormatEnum templateFormat,string baseWSQ,string baseimage, string devicefinger,string deviceID)
        {
            using (sqlCon1 = new SqlConnection(SqlconString1))
            {
                sqlCon1.Open();
                SqlCommand sql_cmnd = new SqlCommand("AddFingerPrint", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;
                sql_cmnd.Parameters.AddWithValue("@UserID", SqlDbType.Int).Value = ID;
                sql_cmnd.Parameters.AddWithValue("@fingerIndex", SqlDbType.Binary).Value = fingerIndex;
                sql_cmnd.Parameters.AddWithValue("@flag", SqlDbType.Binary).Value = flag;
                sql_cmnd.Parameters.AddWithValue("@templateFormat", SqlDbType.NVarChar).Value = templateFormat;
                sql_cmnd.Parameters.AddWithValue("@baseWSQ", SqlDbType.NVarChar).Value = baseWSQ;
                sql_cmnd.Parameters.AddWithValue("@baseImage", SqlDbType.NVarChar).Value = baseimage;
                sql_cmnd.Parameters.AddWithValue("@devicefinger", SqlDbType.NVarChar).Value = devicefinger;
                sql_cmnd.Parameters.AddWithValue("@deviceID", SqlDbType.NVarChar).Value = deviceID;
        ...
        public void AddFaceData(int ID, byte faceIndex, byte numOfTemplate,byte flag,int imageLen , byte[] imageData, byte[] templatedata)
        {
            using (sqlCon1 = new SqlConnection(SqlconString1))
            {
                sqlCon1.Open();
                SqlCommand sql_cmnd = new SqlCommand("AddFaceData", sqlCon1);
                sql_cmnd.CommandType = CommandType.StoredProcedure;
                sql_cmnd.Parameters.AddWithValue("@UserID", SqlDbType.Int).Value = ID;
                sql_cmnd.Parameters.AddWithValue("@faceIndex", SqlDbType.Binary).Value = faceIndex;
                sql_cmnd.Parameters.AddWithValue("@numOfTemplate", SqlDbType.Binary).Value = numOfTemplate;
                sql_cmnd.Parameters.AddWithValue("@flag", SqlDbType.Binary).Value = flag;
                sql_cmnd.Parameters.AddWithValue("@imageLen", SqlDbType.Int).Value = imageLen;
                sql_cmnd.Parameters.AddWithValue("@imageData", SqlDbType.Binary).Value = imageData;
                sql_cmnd.Parameters.AddWithValue("@templatedata", SqlDbType.NVarChar).Value = templatedata;
                sql_cmnd.ExecuteNonQuery();
                sqlCon1.Close();
            }
        }
        ...
        public bool AddUserBlob(ref BS2UserBlob userBlob, BS2FingerprintTemplateFormatEnum templateFormat)
        {
            ...
            if (userName.Length > 0)
            {
                ...
                cmd.Parameters.AddWithValue("@userNameParam", userBlob.name);
                 ...
                 id = Encoding.UTF8.GetString(userBlob.user.userID).TrimEnd('\0');
                string name = System.Text.Encoding.UTF8.GetString(userBlob.name);
                string start = userBlob.setting.startTime.ToString();
                string end = userBlob.setting.endTime.ToString();
                
                AddUser1(int.Parse(Encoding.UTF8.GetString(userBlob.user.userID).TrimEnd('\0')), name,start,end);
                if (cmd.ExecuteNonQuery() < 1)
                {
                    transaction.Rollback();
                    return false;
            ...
            if (userBlob.photo.size > 0)
            {
                ...
                AddPhotoImage(int.Parse(Encoding.UTF8.GetString(userBlob.user.userID).TrimEnd('\0')), (int)userBlob.photo.size, userBlob.photo.data);
            ...
            if (!isEmptyPinCode)
            {
                ...
                AddPin(int.Parse(Encoding.UTF8.GetString(userBlob.user.userID).TrimEnd('\0')), userBlob.pin);
            ...
            if (userBlob.user.numFingers > 0)
            {
                ...
                for (byte idx = 0; idx < userBlob.user.numFingers; ++idx)
                {
                    ...
                    string base64 = Util.ToBase64String(finger.fingerImage,ImageFormat.Jpeg);
                    string base64WSQ = Util.GetBase64ImageWSQ(finger.fingerImage);                    
                    string base64byte = Util.GetBase64frombyte(finger.data);

                    AddFingerPrint(int.Parse(Encoding.UTF8.GetString(userBlob.user.userID).TrimEnd('\0')), finger.index, finger.flag, templateFormat, base64WSQ, base64, base64byte, finger.deviceID.ToString());
            ...
            if (userBlob.user.numFaces > 0)
            {
                ...
                for (byte idx = 0; idx < userBlob.user.numFaces; ++idx)
                {
                    ...
                    AddFaceData(int.Parse(id), face.faceIndex, face.numOfTemplate, face.flag, face.imageLen, face.imageData, face.templateData);
                    if (cmd.ExecuteNonQuery() < 1)
                    {
                        transaction.Rollback();
                        return false;
    ...
    public class UserControl : FunctionModule
    {
        ...
        public void insertUserIntoDatabase(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            BS2FingerprintTemplateFormatEnum templateFormat = BS2FingerprintTemplateFormatEnum.FORMAT_ANSI;
           ...
           // bool faceScanSupported = Convert.ToBoolean(deviceInfoEx.supported & (UInt32)BS2SupportedInfoMask.BS2_SUPPORT_FACE_EX_SCAN);
            ...
            if (fingerScanSupported)
            {
                ...
                if (userBlob.user.numFingers > 0)
                {
                    ...
                    for (int idx = 0; idx < userBlob.user.numFingers; ++idx)
                    {
                        ...
                        API.BS2_GetLastFingerprintImage( sdkContext,  deviceID, out  imageObj, out  imageWidth, out  imageHeight);
                        
                        int size = (int)imageWidth * (int)imageHeight;
                        byte[] managedArray = new byte[size];
                        Marshal.Copy(imageObj, managedArray, 0, size);
                        fingerprint.fingerImage = getimage(managedArray,(int)imageWidth , (int)imageHeight,0);
        ...
        public void getUserExFromDevice(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            Console.WriteLine("Trying to enroll user.");
            if (!dbHandler.AddUserBlobEx(ref userBlobs[0], BS2FingerprintTemplateFormatEnum.FORMAT_ANSI))
            {
                Console.WriteLine("Can not enroll user in the system.");
            }
        ...
        { 
            ...
            using (SqlConnection con = new SqlConnection(SqlconString1))
            {
                string oString = "SELECT userID, userName from BS2UserName";
                using (SqlCommand cmd = new SqlCommand(oString, con))
                {
                    //cmd.CommandType = CommandType.StoredProcedure;
                    //cmd.Parameters.AddWithValue("@userIDParam", usertarget);
                    ...
                    using (SqlDataReader oReader = cmd.ExecuteReader())
                    {
                        while (oReader.Read())
                        {
                            ...
                            Console.WriteLine("When is this user valid from? [default(Today), yyyy-MM-dd HH:mm:ss]");
                            Console.Write(">> ");
                            if (!Util.GetTimestamp2("yyyy-MM-dd HH:mm:ss", 0, out userBlob[0].setting.startTime))
                            {
                                return;
                            }
                            ...
                            Console.WriteLine("When is this user valid to? [default(Today), yyyy-MM-dd HH:mm:ss]");
                            Console.Write(">> ");
                            if (!Util.GetTimestamp2("yyyy-MM-dd HH:mm:ss", 0, out userBlob[0].setting.endTime))
                            {
                                return;
                            }
...

                            {
                                ...
                                Console.Write(">> ");
                                userBlob[0].setting.securityLevel = Util.GetInput2((byte)BS2UserSecurityLevelEnum.DEFAULT);
                            }
...

                            {
                                ...
                                Console.Write(">> ");
                                userBlob[0].user.authGroupID = (UInt32)Util.GetInput2(0);
                            }

                            {
                                Console.WriteLine("Do you want to overwrite the user if it exist? [y/n]");
                                Console.Write(">> ");
                                userBlob[0].user.flag = (byte)(Util.IsYes2face() ? BS2UserFlagEnum.CREATED | BS2UserFlagEnum.UPDATED : BS2UserFlagEnum.CREATED);
                            }
                                    ...
                                    if (0 < numOfFace)
                                    {
                                        ...
                                        for (int index = 0; index < numOfFace;)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                string base64String = Convert.ToBase64String(faceEx[index].imageData, 0, (int)faceEx[index].imageLen);
                                                userBlob[0].user.numFaces++;
                                                index++;
